...
Odometry::Odometry(size_t velocity_rolling_window_size)
    : timestamp_(0.0), x_(0.0), y_(0.0), heading_(0.0), linear_x_(0.0),
      linear_y_(0.0), angular_(0.0), wheel_separation_(0.0),
...
bool Odometry::update(double left_front_pos, double left_rear_pos,
  ...
  linear_acc_x_(X(0) / dt);
  linear_acc_y_(X(1) / dt);
  angular_acc_(X(2) / dt);
...
void Odometry::resetAccumulators() {
  linear_acc_x_ = RollingMeanAcc(RollingWindow::window_size =
                                     velocity_rolling_window_size_);
  linear_acc_y_ = RollingMeanAcc(RollingWindow::window_size =
                                     velocity_rolling_window_size_);
